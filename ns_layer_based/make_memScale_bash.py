...
for nt in nts:
    with open(f'ms_{nt}.sh', 'w') as f:
        ...
        f.write(f'./multi_ns_parallel -p b_{str(nt).zfill(2)} -t FATAL --caseID=ms_{str(nt).zfill(2)} -f -n {nt} 1')

with open('runMemScale.sh', 'w') as f:
    f.write(f'#!/bin/bash\n\n')
    f.write(f'for i in {" ".join((str(i) for i in (nts)))}\n')
    f.write('do\n')
    f.write('    sbatch ms_$i.sh\n')
    f.write('done')
    
os.system('chmod +x runMemScale.sh')